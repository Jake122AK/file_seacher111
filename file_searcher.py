...
class Indexer:
    ...
    def search(self, query: str, mode: str = "AND",
        ...
        mode: "AND" = 全部含む / "OR" = いずれか含む。
        除外語(-keyword)は mode に関係なく常に NOT として動作。
        日本語2文字以下のキーワードは trigram で扱えないため LIKE で補完。
        """
        if not query.strip():
            return []
        ...
        try:
            ...
            # FTS可（3文字以上）と LIKE のみ（2文字以下）に振り分け
            ...
            # === Step 2: OR モードで、短い肯定語がある場合は LIKE 検索を追加 ===
                ...
                for path, name, content in like_rows:
                    ...
                    for t in like_pos:
                        ...
                        if t.lower() in name.lower():
                            snip = name
                            break
                    candidates.append((path, name, snip))
                    seen_paths.add(path)

            # === Step 3: AND モードで FTS 語がない場合は全件走査 ===
            ...
            # === Step 4: 後処理フィルタ ===
            #   - AND モードで FTS 語と短い語が両方ある場合：候補に短い語が全部含まれるか確認
            #   - 短い否定語：候補に含まれていれば除外
            need_post_filter = (
            ...
            if need_post_filter and candidates:
                ...
                filtered = []
                for path, name, snip in candidates:
                    body = contents_map.get(path, "")
                    haystack = (body + "\n" + name).lower()
                    # AND モード：短い肯定語が全部含まれるか
