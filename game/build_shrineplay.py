    ...
    if(open > 0.003){
      ...
      if(sector > 0.002){
        ...
        /* 色はその画素の空に**掛けて**作る。加算や定数置き換えにすると露出と
           ...
           効かせ方は「明暗の境を作る」こと。地平付近の空は uHazeAmt=0.86 で
           霧色に潰れているうえ、合成パスで散乱光が加算される。そこへ 10% の
           濃淡を置いても、ただの靄にしか見えない（実際そうなっていた）。
           空 → 暗い稜線 → 明るい盆地、と交互にすると、目が輪郭を拾う。 */
        /* 振れ幅の目安は実測から決めた。この帯にはボリューメトリックの散乱光が
           合成パスで **0.19** ほど加算される（空の部分は 0.03）。ライティングの
           出力を 10% 動かしても、合計 0.30 に対する 0.02 なので目には出ない。
           実際、0.44 -> 0.34 に落としても絵は1ピクセルも変わらなかった。
           加算に対して勝てる比（3倍前後）を作りにいく。 */
        vec3 mnt = sky * vec3(0.10, 0.15, 0.26);           // 西山。落とし切る
        ...
        /* 陽の当たった盆地は、地平の空より**明るい**。山の上から見下ろすと
           実際にそうなるし、暗い稜線との境がそのまま輪郭になる。 */
        vec3 town = sky * mix(vec3(3.30,3.18,2.92), vec3(1.85,1.79,1.66), dep)
                        * (0.80 + 0.42*grain);
        town += vec3(0.115,0.078,0.032)*smoothstep(0.52,0.90,grain)*uStarAmt*3.4;
        ...
        vec3 hill = sky * vec3(0.26, 0.30, 0.30);
