...
function loadGLB(buf, status) {
  ...
  /* プリミティブは**全部**読む。1本目だけ読んでいたので、材質ごとに分けて
     書き出されたモデル（glTF は材質がプリミティブ単位）は、1材質ぶんの
     面しか出ず、体がばらばらに見えていた。
  ...
  const nrm = A.NORMAL !== undefined ? glbAcc(G, A.NORMAL).slice() : null;
  const ji = Array.from(glbAcc(G, A.JOINTS_0));
  const jw = Array.from(glbNorm(glbAcc(G, A.WEIGHTS_0),
                                J.accessors[A.WEIGHTS_0].componentType));
  const idx = Array.from(glbAcc(G, prim.indices));
  let NV = pos.length / 3;
  for (let k = 1; k < prims.length; k++) {
    const p2 = prims[k], A2 = p2.attributes;
    const same = A2.POSITION === A.POSITION;
    const base = same ? 0 : NV;
    if (!same) {
      const p = glbAcc(G, A2.POSITION), n = A2.NORMAL !== undefined ? glbAcc(G, A2.NORMAL) : null;
      const j = glbAcc(G, A2.JOINTS_0);
      const w = glbNorm(glbAcc(G, A2.WEIGHTS_0), J.accessors[A2.WEIGHTS_0].componentType);
      for (let i = 0; i < p.length; i++) pos.push(p[i]);
      if (nrm) for (let i = 0; i < p.length; i++) nrm.push(n ? n[i] : (i % 3 === 1 ? 1 : 0));
      for (let i = 0; i < j.length; i++) { ji.push(j[i]); jw.push(w[i]); }
      NV += p.length / 3;
    }
