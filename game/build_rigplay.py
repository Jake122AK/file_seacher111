...
function findLandmarks(P, IDX) {
  ...
  const bx = new Array(NB);
  for (let b = 0; b < NB; b++) bx[b] = [];
  ...
  for (let i = 0; i < NV; i++) {
    ...
    bx[b].push(Math.abs(x));
    if (x > 0) {                                       // the right leg only
      if (x < rx0[b]) rx0[b] = x; if (x > rx1[b]) rx1[b] = x;
      if (z < rz0[b]) rz0[b] = z; if (z > rz1[b]) rz1[b] = z;
    }
  }
  /* trunk width : NOT the widest thing in the band. Sort |x| and cut at
     the first gap wider than 3.5% of stature — everything past that gap is
     a separate limb. Taking the plain maximum inside a fixed window made
     the WAIST come out below the CROTCH on an A-pose figure, because a
     hand resting beside the hip is the widest thing at that height. */
  for (let b = 0; b < NB; b++) {
    const a = bx[b];
  ...
  /* 股下 : the highest band that still contains TWO separate pieces of
     surface.

     Counting vertices near x = 0 does not survive a low-polygon model, and
     that is not a rare case — it is most downloaded meshes. A tube of 18
     segments has its nearest vertices 10 degrees off the midline plane, so
     a perfectly solid torso can have nothing within a centimetre of x = 0
     and read as "legs apart". Measured on a 6,500-vertex test figure, the
     vertex test put the crotch 13cm above the waist.

     Topology does not care about tessellation. Two legs are two rings; a
     torso is one. Slice the mesh into bands and count the components of
     each band, restricted to a column wide enough to exclude arms hanging
     beside the hips but not so narrow it cuts a torso into front and back. */
  const COL = H * 0.16;
  const inCol = new Uint8Array(NV);
  for (let i = 0; i < NV; i++) inCol[i] = Math.abs(P[i * 3]) < COL ? 1 : 0;
  const fBand = new Array(NB);
  for (let b = 0; b < NB; b++) fBand[b] = [];
  for (let t = 0; t < IDX.length; t += 3) {
    const a = IDX[t], b2 = IDX[t + 1], c = IDX[t + 2];
    if (!inCol[a] || !inCol[b2] || !inCol[c]) continue;
    const y = (P[a * 3 + 1] + P[b2 * 3 + 1] + P[c * 3 + 1]) / 3;
    fBand[bOf(y)].push(t);
  }
  const par2 = new Int32Array(NV);
  const find2 = a => { while (par2[a] !== a) { par2[a] = par2[par2[a]]; a = par2[a]; } return a; };
  /* Two pieces is not enough on its own: a clothed model has a body shell
     and a garment shell in the same band, and counting alone reads that as
     a pair of legs — which put the reference figure's crotch 16cm above
     its waist. Legs are two pieces SIDE BY SIDE, so the test is a gap in x
     that straddles the midline. A garment overlaps the body in x and never
     produces one. */
  const legsApart = b => {
    const F = fBand[b];
    if (F.length < 6) return false;
    for (const t of F) for (let k = 0; k < 3; k++) par2[IDX[t + k]] = IDX[t + k];
    for (const t of F) for (let k = 1; k < 3; k++) {
      const x = find2(IDX[t]), y = find2(IDX[t + k]);
      if (x !== y) par2[y] = x;
    }
    const cm = new Map();
    for (const t of F) for (let k = 0; k < 3; k++) {
      const v = IDX[t + k], r = find2(v), x = P[v * 3];
      let e = cm.get(r);
      if (!e) { e = { n: 0, lo: 1e9, hi: -1e9 }; cm.set(r, e); }
      e.n++; if (x < e.lo) e.lo = x; if (x > e.hi) e.hi = x;
    }
    const cs = [...cm.values()].filter(e => e.n >= 9).sort((a, b2) => a.lo - b2.lo);
    for (let i = 0; i + 1 < cs.length; i++)
      if (cs[i].hi < 0 && cs[i + 1].lo > 0) return true;
    return false;
  };
  let bCrotch = -1;
  for (let b = 1; b < NB * 0.60; b++) if (legsApart(b)) bCrotch = b;
  /* a skirt, a mermaid tail, a single fused leg — nothing ever splits, so
     there is no crotch to find and the anatomical fraction is the answer */
  if (bCrotch < 0) bCrotch = Math.round(NB * 0.46);
  bCrotch = clamp(bCrotch, Math.round(NB * 0.30), Math.round(NB * 0.56));
...
function findExtras(P, IDX, L) {
  ...
  for (const s of [-1, 1]) {
    ...
    /* The thumb is at one END of the fan AND starts nearest the wrist.
       Either test alone fails: "nearest the wrist" by a fixed margin missed
       it on a figure whose whole hand starts early (every digit inside
       0.22-0.32 along the hand), and "outermost" alone would promote a
       little finger on a hand that has no thumb at all. */
    const ends = [made[0], made[made.length - 1]];
    const th = ends[0].baseT <= ends[1].baseT ? ends[0] : ends[1];
    let lowest = true;
    for (const f of made) if (f !== th && f.baseT < th.baseT) lowest = false;
    if (made.length >= 4 && lowest) th.thumb = true;
...
function loadModel(text, status) {
  ...
  const NV = P.length / 3;
  if (!NV || !IDX.length) { status('このファイルからは頂点と面が読めませんでした。Wavefront .obj を渡してください（.fbx や .glb、zip のままでは読めません）'); return false; }

  status('骨格を推定中…');
  const L = findLandmarks(P, IDX);
...
function setupInput() {
  ...
  const mid = e => ({ x: (e.touches[0].clientX + e.touches[1].clientX) / 2,
                      y: (e.touches[0].clientY + e.touches[1].clientY) / 2 });
  let two = null;
  c.addEventListener('mousedown', e => { drag = pos(e); drag.pan = e.shiftKey || e.button === 2; });
  c.addEventListener('contextmenu', e => e.preventDefault());
  c.addEventListener('touchstart', e => {
    if (e.touches.length > 1) { pinch = d2(e); two = mid(e); drag = null; }
    else { drag = pos(e); drag.pan = false; }
  }, { passive: true });
  const move = e => {
    if (e.touches && e.touches.length > 1) {
      /* pinch zooms, and the same two fingers moving together pan — there is
         no shift key on a phone and the hands are 60cm off the axis */
      const d = d2(e), m = mid(e);
      if (pinch) ORB.distT = clamp(ORB.distT * (pinch / d), 0.3, 14);
      if (two) {
        ORB.txT = clamp(ORB.txT - (m.x - two.x) * ORB.dist * 0.0016, -2, 2);
        ORB.tyT = clamp(ORB.tyT + (m.y - two.y) * ORB.dist * 0.0016, -0.5, 2.4);
      }
      pinch = d; two = m; e.preventDefault(); return;
    }
    if (!drag) return;
    const p = pos(e);
    const pan = drag.pan;
  ...
  addEventListener('touchend', () => { drag = null; pinch = 0; two = null; });
...
function readFile(f) {
  /* no extension test. A phone hands over "model.obj.txt", a download comes
     back as "scene (1).obj", and the only thing that actually settles it is
     whether the text parses into vertices and faces. */
  status('読み込み中… ' + f.name);
    ...
    setTimeout(() => {
      try {
        if (loadModel(r.result, status)) {
          ...
          /* say what was FOUND, not just what was loaded — the whole rig is
             guessed, so the guess is the interesting part */
          const X = RIG.X, got = [];
          const nf = X.fingers.length;
          if (nf) got.push('指' + nf + '本'); else got.push('指なし');
          if (X.jaw) got.push('顎');
          if (X.eyes.length) got.push('目');
          document.getElementById('info').textContent =
            f.name + ' — ' + RIG.tris.toLocaleString() + ' 三角形 / '
            + RIG.SK.B.length + ' ボーン（' + got.join('・') + '）';
          poseRig(0);
        }
...
async function boot() {
  ...
  setClip('walk');
  /* there is no drag and drop on a touch screen, and telling someone to drop
     a file on a phone is just a dead end */
  if (!matchMedia('(hover: hover)').matches)
    document.getElementById('dropHead').textContent = '.obj ファイルを選んでください';
...
/* Waiting for 'load' is only correct if this script is parsed BEFORE the
   event fires. Embedded somewhere that injects the markup afterwards, the
   listener is attached to an event that has already gone by and boot() is
   never called — the page then sits on "初期化中…" forever, which looks
   exactly like a hang and gives nobody anything to go on. */
function bootOnce() {
  if (window.__booted) return;
  window.__booted = true;
  boot().catch(e => fatal(e && e.message ? e.message : String(e)));
}
function fatal(msg) {
  const st = document.getElementById('status');
  if (st) st.innerHTML = '起動に失敗しました<br><span style="color:#c98">' +
    String(msg).replace(/[<&]/g, c => c === '<' ? '&lt;' : '&amp;') + '</span>';
}
addEventListener('error', e => fatal(e.message || 'スクリプトエラー'));
if (document.readyState === 'complete' || document.readyState === 'interactive') bootOnce();
else addEventListener('load', bootOnce);
/* and if neither path ever gets there, say so rather than spinning */
setTimeout(() => {
  if (!window.__ready) {
    const st = document.getElementById('status');
    if (st && /初期化中|準備/.test(st.textContent))
      st.textContent = '初期化が終わりません。WebGL2 が使えないか、GPU が拒否している可能性があります。';
  }
}, 25000);
'''
...
<style>
  *{box-sizing:border-box}
  ...
  #drop label.f{pointer-events:auto;font-size:13px;padding:10px 22px}
  /* the styled label IS the button; the input itself must not show, but
     display:none makes it unfocusable on some browsers */
...
<div id="ui">
  <div id="drop">
    <div class="big" id="dropHead">.obj を、ここに落としてください</div>
